...
public static void mengurutkanDataSelection(formatBiodata biodataMahasiswa[]) {
    ...
    for (int i = 0; i < N - 1; i++) {
        posisiMaks = i;
        for (int j = i + 1; j < N; j++) {
            // Descending: IPK terbesar ke atas
            if (biodataMahasiswa[j].ipk > biodataMahasiswa[posisiMaks].ipk) {
                posisiMaks = j;
            }
        }
        // Tukar data
        biodataSementara = biodataMahasiswa[i];
        biodataMahasiswa[i] = biodataMahasiswa[posisiMaks];
        biodataMahasiswa[posisiMaks] = biodataSementara;
    }
}
